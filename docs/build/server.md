# Where to host your worlds

On the open web, so you're in full control.<br>
You can test your room-URLs with any janusweb-viewer:

<input type="text" list="janusweb" style="width: 100%; max-width: 800px; padding: 10px; font-size: 21px; color: #888; border-radius: 8px;" placeholder="doubleclick to select server" />
<datalist id="janusweb">
  <option>web.janusxr.org/#janus.url=</option>
  <option>xrforge.isvery.ninja/view/index.html#janus.url=</option>
</datalist>

1. select a server above (doubleclick)
2. copy-paste your URL after `#janus.url=` 
3. copy-paste the whole URL into a new browsertab (+Profit!)
...
| ❤️[janusxr.com](tools.md) | selfhost / desktop / intranet | convenient | also supports lua (see [redbean](https://redbean.dev)) |
...
Presence (syncing user positions) is totally optional, and done via a presence server:

* The protocol is fully open (see below) 
* anyone can make one 
* you can write extensions for it

|                    | repo | info      |
|--------------------|------|-----------|
| janus-server (javascript)  | [link](https://github.com/janusvr/janus-server) | most mature / extendable server using hooks |
| janus server (python3) | [link](https://github.com/Spyduck/VRPresence) | lightweight |

> click [here for more server-related server/client-tools](https://github.com/search?q=janusvr%20server&type=repositories)
...
- **urlContains = {domainname}** - Only displays entries that contain a certain url. For instance, ?urlContains=vesta.janusxr.org.
